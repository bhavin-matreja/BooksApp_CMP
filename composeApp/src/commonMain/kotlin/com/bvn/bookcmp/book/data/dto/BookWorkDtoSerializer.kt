...
object BookWorkDtoSerializer : KSerializer<BookWorkDto> {
    ...
    // decode the desried object into BookWorkDto
    override fun deserialize(decoder: Decoder): BookWorkDto
        ...
        while (true) {
            when (val index = decodeElementIndex(descriptor)) {
                0 -> {
                    val jsonDecoder = decoder as? JsonDecoder ?: throw
                            SerializationException("This decoder only works with JSON")
    ...
    /* If you have bookWorkDto and want to push that to API, then
     take kotin object and serialize it to JSON
     Note: It wont be called in our app
     */
